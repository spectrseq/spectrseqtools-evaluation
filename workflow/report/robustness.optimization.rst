...
Prediction results for experimental data over different parameters.
Default settings are a 75th percentile intensity cutoff, 60s for the full
and 5s for the short solving of a linear program.
For the evaluation, all parameters are set to their default value except the
one being evaluated.
...
tolerance ("identical (minus 55U/G)"), correct composition in the wrong order ("correct composition"), correct length
